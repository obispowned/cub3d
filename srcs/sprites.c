	...
	while (mlx->rc.stripe < mlx->rc.drawendx)
	{
		...
		if (mlx->rc.transformy > 0 && mlx->rc.stripe > 0 && mlx->rc.stripe < mlx->win_width &&
			...
			while (y < mlx->rc.drawendy)
			{
				...
				if (mlx->rc.sprite[i].tex.data[4 * (mlx->rc.sprtexx + 64 * mlx->rc.sprtexy)]
				!= (char)0)
					ft_memcpy(mlx->img + 4 * mlx->win_width * y + mlx->rc.stripe * 4,
						&mlx->rc.sprite[i].tex.data[mlx->rc.sprtexy % mlx->rc.tex_height *
						mlx->rc.sprite[i].tex.size_l + mlx->rc.sprtexx % mlx->rc.tex_width
