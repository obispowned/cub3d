...
void	sprite_casting2(t_mlx *mlx) /*adapptation*/
{
	...
	mlx->rc.spritescreenx = (int)((mlx->rc.win_x / 2) * (1 + mlx->rc.transformx /
						mlx->rc.transformy));
	mlx->rc.spriteheight = abs((int)(mlx->rc.win_y / (mlx->rc.transformy)));
	mlx->rc.drawstarty = -mlx->rc.spriteheight / 2 + mlx->rc.win_y / 2;
	if (mlx->rc.drawstarty < 0)
		mlx->rc.drawstarty = 0;
	mlx->rc.drawendy = mlx->rc.spriteheight / 2 + mlx->rc.win_y / 2;
	if (mlx->rc.drawendy >= mlx->rc.win_y)
		mlx->rc.drawendy = mlx->rc.win_y - 1;
	mlx->rc.spritewidth = abs((int)(mlx->rc.win_y / (mlx->rc.transformy)));
	...
	if (mlx->rc.drawendx >= mlx->rc.win_x)
		mlx->rc.drawendx = mlx->rc.win_x - 1;
}
